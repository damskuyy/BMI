    ...
    <div class="card">
        <div class="card-body">
            <div class="form-card">
                ...
                <form action="{{ route('product_be.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="section-grid">
                                @foreach(['manufaktur' => 'Manufaktur', 'kuliner' => 'Kuliner (UMKM)', 'kerajinan' => 'Kerajinan'] as $key => $label)
                                    <label class="option-btn {{ old('category', $product->category) === $key ? 'checked' : '' }}">
                                        <input type="radio" name="category" value="{{ $key }}" {{ old('category', $product->category) === $key ? 'checked' : '' }} required>
                                        <div>{{ $label }}</div>
                                    </label>
                                @endforeach
                            </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="section-grid">
                            <label class="option-btn {{ old('ordering_method', $product->ordering_method) === 'marketplace' ? 'checked' : '' }}">
                                <input type="radio" name="ordering_method" value="marketplace" {{ old('ordering_method', $product->ordering_method) === 'marketplace' ? 'checked' : '' }} required>
                                <div>🛍️ Marketplace</div>
                            </label>
                            <label class="option-btn {{ old('ordering_method', $product->ordering_method) === 'whatsapp' ? 'checked' : '' }}">
                                <input type="radio" name="ordering_method" value="whatsapp" {{ old('ordering_method', $product->ordering_method) === 'whatsapp' ? 'checked' : '' }} required>
                                <div>💬 WhatsApp</div>
                            </label>
                        </div>
                        ...
                        <!-- Marketplace Links -->
                        <div class="conditional-field {{ old('ordering_method', $product->ordering_method) === 'marketplace' ? 'show' : '' }}" id="marketplaceFields">
                        ...
                        <!-- WhatsApp Phone -->
                        <div class="conditional-field {{ old('ordering_method', $product->ordering_method) === 'whatsapp' ? 'show' : '' }}" id="whatsappFields">
                            <label class="form-label">Contact Number</label>
                            <div class="phone-button-group">
                                <button type="button" class="phone-btn {{ old('use_default_phone', $product->use_default_phone) ? 'active' : '' }}" onclick="setPhoneMode(true, event)">
                                    Use Default (62851-xxx-xxxx)
                                </button>
                                <button type="button" class="phone-btn {{ !old('use_default_phone', $product->use_default_phone) ? 'active' : '' }}" onclick="setPhoneMode(false, event)">
                                    Custom Number
                                </button>
                            </div>
                            <input type="hidden" id="use_default_phone" name="use_default_phone" value="{{ old('use_default_phone', (int)$product->use_default_phone) }}">

                            <div id="customPhoneField" class="mt-3" style="{{ old('use_default_phone', $product->use_default_phone) ? 'display:none;' : '' }}">
                                <input 
                                    type="text" 
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                if (!imagePreview) {
                    ...
                    imagePreviewBox.appendChild(newImg);
                }
